const fs = require('fs');
const content = fs.readFileSync('tra-pids.html', 'utf8');

const regex = /\s*\/\* [^\*]*RWD 修正區塊[^\*]*\*\/\s*@media \(max-width: 699px\)[\s\S]*?<\/style>/;
const match = content.match(regex);
if (!match) {
    const backupRegex = /\s*@media \(max-width: 699px\)[\s\S]*?<\/style>/;
...
const replaceStr =         /* 🔥 手機版/直向螢幕 終極優化 (針對 iPhone/Android 螢幕) */
        @media (max-width: 699px) {
            ...
            #videoBox { display: none; } /* 手機版隱藏影片，避免卡頓，只留圖片宣導 */
...
const newContent = content.replace(regex, '\\n' + replaceStr).replace(/\\s*@media \\(max-width: 699px\\)[\\s\\S]*?<\\/style>/, '\\n' + replaceStr);
fs.writeFileSync('tra-pids.html', newContent);
