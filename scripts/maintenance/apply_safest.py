...
# 1. Variables
if 'let currentMode = "station";' not in text:
    text = text.replace(
        'let allStationsFlat = [];',
        'let allStationsFlat = [];\n        let currentMode = "station";\n        let isAutoRefreshEnabled = false;'
    )

# 2. Time Update config removal
import re
text = re.sub(r'// 時鐘更新\s*function updateTime\(\) \{[\s\S]*?\}\s*setInterval\(updateTime,\s*1000\);\s*', '', text)
text = re.sub(r'// 通阻一分鐘更新一次\s*setInterval\(\(\) => loadTraAlerts\(\),\s*60000\);\s*', '', text)

# 3. DOMContentLoaded injection
if "startHeartbeat();" not in text:
    dc_sig = "document.addEventListener('DOMContentLoaded', () => {"
    dc_close = "});"
    
    start_idx = text.find(dc_sig)
    end_idx = text.find(dc_close, start_idx) + 3
    
    dc_block = text[start_idx:end_idx]
    new_dc_block = dc_block[:-3] + "    startHeartbeat(); // 🔥 啟動指揮官\n        });"
    ...
    text = text[:start_idx] + new_dc_block + hb_code + text[end_idx:]

# 4. Mode Switch Update
sw_start = text.find('const originalSwitchSearchMode = function(mode) {')
sw_end = text.find('        // ============ 公告載入 (完整原版還原 + 串接 Worker) ============')

if sw_start != -1 and sw_end != -1:
